...
系統使用輕量化關聯式資料庫 `PhotoAI.sqlite`，存放於應用程式儲存目錄(沙盒環境)中。資料庫內共包含以下四張核心資料表，用以維護相片特徵、相簿分群及 AI 筆記 ：

### 1. 相片主表 (`PhotoFeatures`)
儲存相片的唯一識別碼、拍攝元資料、512 維影像嵌入向量、所屬分類編號與最愛旗標 。
| 主要欄位 | 說明 |
| --- | --- |
| **相片唯一識別碼** | 相片的主鍵 (Primary Key) |
| **元資料 (JSON 格式)** |儲存相片的拍攝詮釋資料（如時間、經緯度等） |
| **512維影像嵌入向量** | 由 CoreML 模型推論產生的正規化影像特徵向量 |
| **所屬分類編號** | 關聯至 `ClusterSummary` 的分類代碼  |
| **最愛旗標 (0/1)** | 用於標記使用者是否點選喜愛該照片 |

### 2. 相簿分類表 (`ClusterSummary`)
以關鍵字 (keyword) 作為子相簿名稱，並定義其對應的主分類群組 。
| 主要欄位 | 說明 |
| --- | --- |
| **分類編號** | 分類的主鍵 (Primary Key)  |
| **子相簿關鍵字** | 作為子相簿的名稱（如特定主題關鍵字） |
| **主分類名稱** | 對應的主分類（例如：旅遊、美食、寵物等群組）  |

### 3. 語意字典表 (`SemanticDictionary`)
每個語意關鍵字對應一個預先計算的 512 維文字嵌入向量，作為系統自動分類的比對基準 。
| 主要欄位 | 說明 |
| --- | --- |
| **語意關鍵字** | 語意字詞的主鍵 (Primary Key) ] |
| **對應的512維文字嵌入向量** | 預先產生的文字特徵向量，用於本地端快速向量比對 |

### 4. AI 筆記表 (`AgentNotes`)
[cite_start]記錄由 AI Agent 產生的問答格式摘要與筆記，附圖以高壓縮比 JPEG 實體存於應用程式 Documents 目錄中 。
| 主要欄位 | 說明 |
| --- | --- |
| **筆記唯一識別碼** | 筆記的主鍵 (Primary Key) |
| **標題** | ]筆記的標題內容  |
| **內容** | AI Agent 產生的問答式或摘要內文  |
| **附圖檔名清單** | 以逗號分隔的 JPEG 檔名清單 |
| **最愛旗標 (0/1)** | 筆記的最愛狀態旗標  |
| **建立時間** | 該筆筆記的建立時間戳記  |
---
